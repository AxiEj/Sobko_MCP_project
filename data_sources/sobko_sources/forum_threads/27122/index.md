---
thread_id: 27122
source_id: forum_thread:27122
title: "CP2K做杂化泛函计算的关键要点和简单例子"
url: http://bbs.keinsci.com/thread-27122-1-1.html
...
source_crawled_at: "2026-06-05T09:50:25.238Z"
...
authority_level: B
confidence: 0.97
classification_reason: sobereva原创教程，讲解CP2K中杂化泛函计算的关键要点。
---

# CP2K做杂化泛函计算的关键要点和简单例子

- 原帖 URL：<http://bbs.keinsci.com/thread-27122-1-1.html>
- 论坛板块：第一性原理
- 作者：**sobereva**
- 浏览量：10315 | 回复数：10 | 共1页
- 完整性：**全部内容已完整抓取**。
...
CP2K做杂化泛函计算的关键要点和简单例子Key points and simple examples of hybrid functional calculations with CP2K

文/Sobereva@北京科音First release: 2023-Nov-7   Last update: 2024-Feb-18
...
本文只是讲一些最基础的内容。我讲授的相关 CP2K 第一性原理教程（http://www.keinsci.com/KFP）的“能量的计算及相关问题”部分里讲杂化泛函的计算远远比本文具体、全面、深入得多，并列举大量经验技巧和例子，还详细介绍HSE06、CAM-B3LYP、wB97X等各种范围分离形式的杂化泛函以及revDSD-PBEP86-D3(BJ)等各种双杂化泛函的用法和要点、使用杂化泛函结合k点计算的要点等等，强烈推荐想系统、深入学习CP2K的读者参加。
...
2024-Feb-18更新：以下之前写的关于k点的说法已经过时。从CP2K 2024.1版开始支持了杂化泛函计算时考虑k点，称为RI-HFXk算法，并且可以无缝结合ADMM降低耗时。此时需要以RI近似计算HF交换部分。不仅能算能量还能做优化（包括变胞）和能带计算。从第3届的“相关 CP2K 第一性原理教程”开始已经加入了RI-HFXk的全面深入的介绍和具体例子，参考http://bbs.keinsci.com/thread-43683-1-1.html。
...
CP2K还支持其开发者独创的ADMM (Auxiliary Density Matrix Methods)方法减少要算的ERI数目从而进一步显著加快杂化泛函的计算，其原理见以下相关 CP2K 第一性原理教程（http://www.keinsci.com/KFP）的ppt。
...
1.png (104.58 KB, 下载次数 Times of downloads: 198)

下载附件 Download



2023-11-7 18:18 上传 Uploaded
...
注意前述这些辅助基组都是对GTH赝势基组用的，千万别做全电子计算时也用它们。要想全电子计算时用ADMM，可以用(aug-)pcseg系列基组作为主基组，在《在线基组和赝势数据库一览》（http://sobereva.com/309）中介绍的BSE基组数据库中有和它匹配的(aug-)admm辅助基组，诸如pcseg-2适合搭配admm-2。pcseg是一类构造得很好的对DFT计算非常划算的基组，《谈谈量子化学中基组的选择》（http://sobereva.com/336）里简要提及了，在相关中级量子化学教程（[已省略培训课程链接]）里讲基组的地方做了具体介绍。
...
最后，这里以C60分子晶体的单点任务作为例子演示一个标准的周期性杂化泛函的计算。如果你还不了解Multiwfn创建CP2K输入文件的功能，必须先阅读《使用Multiwfn非常便利地创建CP2K程序的输入文件》（http://sobereva.com/587）。
...
2.png (213.72 KB, 下载次数 Times of downloads: 209)
...
3.png (24.19 KB, 下载次数 Times of downloads: 199)
...
本文把强大的CP2K高效地做杂化泛函计算的各方面要点进行了介绍，以避免初学者对各种关键信息毫不知情而凭感觉胡用瞎用。本文的内容也充分体现出用Multiwfn产生CP2K输入文件非常容易，令研究者从复杂的输入文件编写中充分解放，但这绝不意味着CP2K的使用者就可以对关键性技术细节毫不知情，否则会各种踩坑、白浪费时间、算出无意义的结果。CP2K用户必须掌握的知识、要领远多于大部分其它计算程序。笔者开设的相关 CP2K 第一性原理教程（http://www.keinsci.com/KFP）可以使得学员完整系统透彻掌握全部这些知识。

### 2 楼

老师好，我想请教一下，用cp2k对周期体系 用杂化泛函做激发态计算时：1、如果已经提前用PBE泛函做了一次激发态计算，此时读取“RESTART.tdwfn”文件还是“RESTART.wfn”作为波函数初猜更合适一些；2、如果只有PBE泛函做基态结构优化的“RESTART.wfn”文件能否用于杂化泛函做激发态计算的波函数初猜？     谢谢老师

### 3 楼

rendong 发表于 2023-12-2 18:44

老师好，我想请教一下，用cp2k对周期体系 用杂化泛函做激发态计算时：1、如果已经提前用PBE泛函做了一次激 ...

1 tdwfn是TDDFT阶段可以读的记录了组态系数的文件，wfn是SCF阶段可以读的记录了轨道展开系数的文件，是两码事



参考相关 CP2K 第一性原理教程（[已省略培训课程链接]）的ppt：
...
202312030058205013..png (101.37 KB, 下载次数 Times of downloads: 174)

下载附件 Download



2023-12-3 00:58 上传 Uploaded
...
2 可以作为杂化泛函做SCF的初猜波函数

### 4 楼

sobereva 发表于 2023-12-3 00:55

1 tdwfn是TDDFT阶段可以读的记录了组态系数的文件，wfn是SCF阶段可以读的记录了轨道展开系数的文件，是两 ...

好嘞，我明白了，谢谢老师

### 5 楼

4核运行cp2k.popt，inp文件&HF里面限制MAX_MEMMORY 1000，还是爆内存，请问社长有什么办法让体系跑起来吗，增加耗时，降低计算速度也可以承受

### 6 楼

neocc 发表于 2024-9-9 13:02

4核运行cp2k.popt，inp文件&HF里面限制MAX_MEMMORY 1000，还是爆内存，请问社长有什么办法让体系跑起来吗， ...

没具体输入文件、机子的物理内存量完全没法说

### 7 楼

“Multiwfn自动指定的计算设置完全合适，对应的是PBE0/DZVP-MOLOPT-SR-GTH结合admm-dzp辅助基组、6埃库仑截断、EPS_SCHWARZ 1E-6、SCREEN_ON_INITIAL_P T、开启OT”，默认设置中应该是没有开启OT要手动开启。

### 8 楼

Lakersruobai 发表于 2024-11-13 16:56

“Multiwfn自动指定的计算设置完全合适，对应的是PBE0/DZVP-MOLOPT-SR-GTH结合admm-dzp辅助基组、6埃库仑截 ...

目前Multiwfn官网上的最新版本满足此规则

### 9 楼

本帖最后由 Uus/pMeC6H4-/キ 于 2024-11-18 13:12 编辑 

如果PRINT_LEVEL设的是MEDIUM，在SCF第一轮算完后就会输出Number of sph. ERI's calculated on the fly和Number of sph. ERI's stored in-core，...

PRINT_LEVEL MEDIUM这个设置影响范围较广，输出文件与LOW相比多出不少信息而略显冗长。实测发现HF计算ERI的相关信息是由&FORCE_EVAL/&DFT/&XC/&HF下的&HF_INFO字段控制输出的，如果PRINT_LEVEL仍用平时的LOW而添加&HF_INFO字段启用之，那输出中也能看到这些信息。



这个小技巧是在如前帖所述试图以杂化泛函用REFTRAJ重算分子动力学的众多snapshot时发现的。此任务作为重算类型仍为MD，此时我这样写输入文件：
...
这就可以每读取参考轨迹里一个snapshot做杂化泛函计算时便输出一次信息。由于不同snapshot结构的差异，需计算的ERI数量会有些小幅波动。

### 10 楼

说到The Kohn Sham matrix is not 100% occupied警告的处理方式，“晶胞较小、矩阵稀疏性差”的情形有具体的判断标准么？



我有一批水溶液体系在做杂化泛函开OT的计算，内外循环的EPS_SCF均设置为5E-7，相应的EPS_DEFAULT和EPS_PGF_ORB均设为(5E-7)^2=2.5E-13。此时盒子尺寸比较小（不过边长都在12埃以上）的体系不会有这个警告，但一个盒子边长17.7埃的体系仍然会有警告。实测此体系在默认设置下SCF收敛、算出一帧单点能和受力平均需要347秒，而开MIN_PAIR_LIST_RADIUS -1时平均得花372秒。





另外，这帖的第二第三张图顺序和原博文里弄反了

### 11 楼

Uus/pMeC6H4-/キ 发表于 2025-2-3 13:18

说到The Kohn Sham matrix is not 100% occupied警告的处理方式，“晶胞较小、矩阵稀疏性差”的情形有具体 ...

和原子的密度有关系，原理上分布得越密越容易出警告（也看具体元素、基组）
