---
thread_id: 3545
source_id: forum_thread:3545
title: '谈谈量子化学中基组的选择'
url: http://bbs.keinsci.com/thread-3545-1-1.html
...
source_crawled_at: '2026-06-05T16:10:00+08:00'
original_reply_count: 128
page_count: 9
views: 205791
software_tags:
- Gaussian
...
authority_level: A
confidence: 0.99
classification_reason: sobereva原创基组选择长篇指南，系统讲解各类基组特点与适用场景，20万浏览，量子化学版核心置顶参考资源。
---

# 谈谈量子化学中基组的选择

- 原帖 URL：<http://bbs.keinsci.com/thread-3545-1-1.html>
- 论坛板块：量子化学 (Quantum Chemistry)
- 作者：**sobereva**（公社社长，管理员）
- 浏览量：205,791 | 回复数：128 | 共9页
- 完整性：**全部9页共125条楼层正文已完整抓取**。

## 核心结论

量子化学计算中基组的选择直接影响计算精度与成本。不同体系（有机/过渡金属）和不同任务（几何优化/能量/光谱/NMR等）需要不同的基组级别。

## 楼层正文

### 1 楼（楼主）｜sobereva

谈谈量子化学中基组的选择On the choice of basis sets in quantum chemistry



文/Sobereva @北京科音First release: 2016-Jun-16   Last update: 2023-May-3

0 前言



量子化学中基组的选择是一门艺术，富有研究经验和量化理论直觉的研究者能够对所研究的问题、体系、要求的精度选择出性价比最高的基组，而初学者则几乎总是基组选得不合适，轻则糟蹋计算资源，重则结果不准确乃至根本没法用。本文目的是提供基组选用的一般性指导意见，但本文并不会把基组系统性地介绍一遍。想对基组有全面系统的了解，非常推荐参加笔者讲授的面向初学者的相关初级量子化学教程（http://www.keinsci.com/KEQC）和内容更深、更全的相关中级量子化学教程（http://www.keinsci.com/KBQC）。本文不怎么涉及赝势基组的选择问题，因为赝势基组的选择笔者另有其它文章：《谈谈赝势基组的选用》（http://sobereva.com/373）。
...
在“相关高级量子化学教程”（[已省略培训课程链接]）中专门有一节“相对论量子化学”，里面特别详细讲解了相对论量子化学计算，对适合相对论计算的全电子基组做了非常全面的盘点并给出了选用建议。
...
终于等到了 Sob老师关于基组选择的guideline了  现在关于泛函、基组的选择齐了，可以召唤神龙啦o(∩_∩)o
...
1.png (152.53 KB, 下载次数 Times of downloads: 243)

下载附件 Download



2022-7-14 09:31 上传 Uploaded
...
本帖最后由 超限制抱怨 于 2023-4-12 13:55 编辑 

sobereva 发表于 2023-4-12 11:46

def2-TZVP自带极化函数

sob老师，我问错了，应该是弥散，加ma弥散和不加精度区别大嘛，耗时更长嘛！计算资源有限，所以想找个性价比最高的

### 99 楼

超限制抱怨 发表于 2023-4-12 13:50
...
别随便借帖子问无关问题，下次直接删帖处理

### 102 楼

本帖最后由 超限制抱怨 于 2023-5-3 08:49 编辑 



sob老师，def2-tzvp和def2-tzvpp的区别是后者给H加了极化嘛，对于含氢转移的单点计算是否需要用到def2-TZVPP级别。

### 103 楼

2.12节提到的crystal与pob介绍，给的链接均已无法访问：
...
超限制抱怨 发表于 2023-5-3 08:47

sob老师，def2-tzvp和def2-tzvpp的区别是后者给H加了极化嘛，对于含氢转移的单点计算是否需要用到def2-TZVP ...
...
1.在计算同一个体系时，不同任务可以使用不同的方法吗？

2.文中最低可接受级别是针对所有任务吗？

3.若是优化使用b3lyp/6-31g*,能量计算用其他更先进的方法，把基组增加两个档次，是基于6-31g*，还是基于该方法的最低可接受级别？

### 107 楼

limen 发表于 2023-5-9 07:17
...
通常只有几何优化、振动分析、跑IRC必须严格用相同基组

2 否

3 最低可接受级别是有可能发文章的底限，但非要用底限就如同穿破衣烂衫上街，会糟严重歧视，而且像样的商场都进不去

### 108 楼

sobereva 发表于 2023-5-10 21:57
...
老师，也就是说不同计算任务之间可以用不同的方法，比如优化用b3lyp，能量用后HF方法？

### 109 楼

limen 发表于 2023-5-10 23:20
...
诸如此文介绍的carbon文章，DFT优化，DLPNO-CCSD(T)算单点

理论设计由18碳环与锂原子构成的电场可控的光学开关

http://sobereva.com/630（http://bbs.keinsci.com/thread-27232-1-1.html）

### 110 楼

sobereva 发表于 2023-5-10 23:35
...
本帖最后由 beefly 于 2024-11-16 02:57 编辑 
...
#p b3lyp/chkbasis guess=read geom=checkpoint

   polar=raman cphf=rdfreq
...
算振动光谱问题，算频率（以及连带得到正则坐标）和算光谱强度可以视为是两个步骤，前者需要能量对核坐标的导数，后者需要能量对其它扰动的导数（如计算红外强度要算对外电场的导数），通常一起做了，但实际上算光谱强度部分也可以在更便宜的基组下做来节约时间，这是相关中级量子化学教程（http://www.keinsci.com/KBQC）里讲振动分析部分说的“两步法”。
