...
source_crawled_at: '2026-06-05T15:50:00+08:00'
...
authority_level: A
confidence: 0.99
classification_reason: sobereva原创长篇综述，系统讲解DFT泛函选择，持续更新至2024年。论坛管理员亲笔，量子化学版核心置顶参考资源。
---

# 简谈量子化学计算中DFT泛函的选择

- 原帖 URL：<http://bbs.keinsci.com/thread-536-1-1.html>
- 论坛板块：量子化学 (Quantum Chemistry)
- 作者：**sobereva**（公社社长，管理员，UID 1）
- 首次发布：2014-12-29，最后更新：2024-10-23
- 浏览量：257,793 | 回复数：91 | 共7页
- 完整性：**全部7页共92条楼层正文已完整抓取**。

## 核心结论

量子化学计算中DFT泛函的选择需根据具体体系和计算目标综合考虑。能量相关精度排序通常为：CCSD(T) > 双杂化泛函 > 恰当选用的普通泛函 > MP2 > HF > 半经验方法。有机体系、过渡金属体系、弱相互作用、光谱计算等不同场景有不同推荐。

## 楼层正文

### 1 楼（楼主）｜sobereva｜2014-12-29 12:27:24

简谈量子化学计算中DFT泛函的选择A brief discussion on the choice of DFT functional in quantum chemistry calculations



文/Sobereva @北京科音First release: 2014-Dec-29   Last update: 2024-Oct-23
...
这些方法的理论方面的知识介绍不在本文范畴内。在相关初级量子化学教程（内容见[已省略培训课程链接]）里我会对本文涉及方法的背景知识进行系统性的介绍使读者更充分地了解他们。在相关中级量子化学教程（内容见[已省略培训课程链接]）里我会进一步展开做更深入全面透彻的讲解，给出更丰富、更具体的推荐，非常可另行参考。
...
• wB97M(2)：2018年提出，是双杂化泛函中性能最优秀的，ORCA 6.0、从Q-Chem 5.2后期版本开始支持
...
注：特别推荐、非常稳、普适性极好的是用w调控后的LC-wPBE，是对每个体系的每个结构专门优化最佳的w参数，见《优化长程校正泛函w参数的简便工具optDFTw》（http://sobereva.com/346），在《相关高级量子化学教程》（[已省略培训课程链接]）里“交换-相关泛函的调控”一节做了特别全面深入的讲解并给了例子，但代价是需要额外花一些时间优化w参数。此外，用ORCA做基于DSD-PBEP86等双杂化泛函的TDDFT的精度和普适性也比用前述那些普通泛函整体好得多，值得尝试，但明显更耗时、更费内存。

• 计算极化率：PBE0或BHandHLYP（大共轭体系建议用后者）。双杂化里DSD-PBEPBE算极化率最佳，仅次于CCSD。

• 计算超极化率：wB97或LC-τHCTH。BHandHLYP和CAM-B3LYP也可以考虑。

注：计算极化率和第一超极化率几乎最理想的是用前述的w调控后的LC-wPBE。
...
如果你要计算的体系含有几百个非氢原子，而且体系较为接近球形，最典型的就是很大的原子团簇、从晶体结构里抠出来的簇模型等，用CP2K是效率最高的选择，比ORCA还要快得多得多，普通双路服务器上对上千原子算单点能也完全不是难事（对于纯泛函甚至能算得动几千原子）。因为CP2K利用平面波作为辅助基，算这样体系的库仑作用耗时远低于量子化学程序。CP2K完全免费、功能强大，结合Multiwfn使用很容易上手，通过相关 CP2K 第一性原理教程（[已省略培训课程链接]）可以迅速掌握。
...
Gaussian计算中分子的振动分析和热力学数据在使用Freq关键词后会一同给出，请问为什么B3lYP和M06-2X一个对频率更加准确一个对热力学数据更加准确？

如果我用M06-2X计算得到接结构来用B3LYP计算分子振动数据，是不是要比同时用B3LYP计算结构和能量更加准确，而参考热力学数据时使用M06-2X计算的结果？
...
热力学数据来自于电子能量以及热力学校正量

B3LYP和M06-2X算热力学校正量这一部分的时候半斤八两，但由于M06-2X算的电子能量比B3LYP准，所以算热力学数据好。

当然你也可以用B3LYP的热力学校正量加到M06-2X的电子能量上，结果也一样好
...
鉴于ORCA 4.1已经支持了wB97M-V，对帖子相应部分内容做了些修改
...
您对def2基组比较推崇， 但从数量上看，def2svp碳原子16个基， 6-31G*15个，可以说是同一档次，  但 6-311G**才18个基，def2tzvp高达31个，计算量大了几倍啊，如果想在def2svp上增加一点，tzvp又算不动，有没有折中的方案？



另外，我原来一直用b3lyp-d3,现在看似乎PBE0也受您推荐，有没有比较优劣的介绍文章？ 我的体系时 过渡金属催化剂 比如 Ti， Rh催化有机反应
...
def 1代里就只有def-TZVP还有存在意义，即弥补def2-SVP和def2-TZVP的空白

还有种做法是用def2-TZVP(-f)，即去掉f极化，这对DFT计算影响不算特别大，但耗时低得多得多

有机体系算弱相互作用B3LYP-D3(BJ)完全没问题，也比PBE0-D3(BJ)好。但近些年有不少可靠的文章指出对于算过渡金属反应相关问题，PBE0-D3(BJ)更佳，因为本身PBE0算这类问题就比B3LYP更强点。
...
不过看到您以前的帖子说 deftzvp对 Mg 有缺陷？ 我的体系恰好有Mg， 另外，def2tzvp（-f）在gaussian里边是怎么输入的？
...
老师好，最近我师妹做了个体系，优化有机大分子，在考虑了B3LYP,PBE0,CAM-B3LYP泛函之后，选择了B3PW91/6-31G(d) 方法进行了结构优化，总觉得不太合适，又没有比较正式的理由~麻烦问一下老师，能否选用这种方法进行优化~谢谢~
...
不要用B3PW91，非主流，也没有什么理由用这个（无论是精度还是流行程度上）

有机体系就用B3LYP-D3(BJ)优化就很适合，非常流行，基本不会被找碴（除了个例，诸如18轮烯、18碳环之类涉及长程大共轭体系，此类情况B3LYP表现不如wB97XD等合理，参考诸如《一篇最全面、系统的研究新颖独特的18碳环的理论文章》http://sobereva.com/524）
...
老师好，我模拟了有机大分子的吸收光谱，如图所示，之后想考虑分子内的电荷转移等特征，我的问题是，从图来看，B3LYP泛函和实验值吻合的稍微好一点，但看这篇博文，B3LYP不太利于描述电荷转移特征。但是如果要用wB97XD泛函，光谱来看和实验值差的较大，想问一下这种情况应该如何处理？麻烦老师。
...
在当前情况下，如果你只是单纯想通过选择泛函来让模拟结果与实验较好吻合而不管其它因素带来的问题，就用B3LYP就完了。
...
谢谢老师，主要是之后想考察激发特征，怕B3LYP会引起假的激发态。
...
卢老师是不是可以考虑抽时间再开一次“最常用的DFT泛函”和“最常用的量子化学软件”投票了，感觉这几年会有不少变化了。另外“最常用的分子动力学软件”等等是不是也可以考虑开一个
...
VASP之类的在周期性里用杂化泛函计算量会暴增而几乎算不动，内存消耗也很大，这事在cp2k里通过ADMM可以一定程度上缓解。大部分情况优化结构，晶格常数之类的都是纯泛函，只有在要精确计算能带结构之类的时候会去用杂化泛函。纯泛函的话几个常用的区别不算很大，PBE在大多数情况下比较合理，+U这事情就另说了。
...
文中说“首选wB97X-V或wB97M-V，其次PBE0-D3(BJ)和MN15”，那请问在仅含有一个Rh原子的配合物中（总共70-100个原子），PBE0-D3(BJ)在优化和计算中跟wB97X-V有哪些差距呢？

另外博文中提到过当弱相互作用明显时B3LYP-D3(BJ)也是一个不错的选择，该体系是不是也可以选用B3LYP-D3(BJ)呢？
...
wB97X-V、wB97M-V准但是慢，而且支持它的梯度的程序少。不过这种过渡金属体系可能HF成分太高了不好，统一用PBE0-D3我觉得就行了。但这个也看看其他人的意见。

有过渡金属的体系，一般认为PBE0-D3总比B3LYP-D3好
...
Phys. Chem. Chem. Phys. 2018, 20, 23175
...
J. Chem. Theory Comput. 2018, 14, 5, 2596-2608

### 54 楼

请问，涉及过渡金属、镧系锕系配合物有关的情况，有没有一些推荐的综述文献呢？

我最近看了一篇超人气的综述：Christopher J. Cramer and Donald G. Truhlar. Density functional theory for transition metals and transition metal chemistry. Phys. Chem. Chem. Phys., 2009, 11, 10757–10816 但是里面感觉都是一些评测文献的杂乱罗列（也可能我没get到它的点）？有没有综述文献能够告诉我们对于过渡金属的各种问题（能量和势垒计算，结构优化，等）分别用什么泛函比较好呢？
...
sob老师，您好，我计算的体系既涉及过渡金属与过渡金属成键，又涉及到过渡金属与惰性气体或主族元素原子的相互作用，请问我用PBE0，TPSSh还是哪个纯泛函比较好呢，谢谢老师
...
r2SCAN-3c对于过渡金属之间成键，以及弱相互作用，描述得都比较不错。用G09的话，就得考虑要以什么为优先考虑。如果不具体区分的话，TPSS-D3(BJ)可以优先考虑。
...
老师，我想用g09研究过渡金属团簇和CO2、惰性气体元素等主族元素构成的体系，主要用泛函优化基态结构，振动分析，轨道分析，单点能计算想用CCSD（T）方法，就不用泛函了，谢谢老师
...
老师，您好，我看了J. Chem. Theory Comput. 2020, 16, 7, 4416–4428关于配合物不同多重度能量差计算的泛函对比，发现M06-2X在计算3d金属配合物能量差时同样比较优秀，那这个泛函能不能使用呢，因为您也说M06-2X是针对主族元素进行参数优化的。我用MN15L计算配合物能量时，有一个多重度的计算试了一些方法SCF始终不收敛，所以想换一个泛函。
...
本帖最后由 Uus/pMeC6H4-/キ 于 2025-6-25 16:19 编辑 
