---
thread_id: 3345
source_id: forum_thread:3345
title: '谈谈隐式溶剂模型下溶解自由能和体系自由能的计算'
url: http://bbs.keinsci.com/thread-3345-1-1.html
...
source_crawled_at: '2026-06-05T16:30:00+08:00'
original_reply_count: 263
page_count: 18
views: 303513
software_tags:
- Gaussian
...
authority_level: A
confidence: 0.99
classification_reason: sobereva原创隐式溶剂模型权威指南，30万浏览，系统讲解SMD等隐式溶剂模型下各类自由能计算方法，量子化学版核心置顶资源。
---

# 谈谈隐式溶剂模型下溶解自由能和体系自由能的计算

- 原帖 URL：<http://bbs.keinsci.com/thread-3345-1-1.html>
- 论坛板块：量子化学 (Quantum Chemistry)
- 作者：**sobereva**（公社社长，管理员）
- 浏览量：303,513 | 回复数：263 | 共18页
- 完整性：**全部18页内容已完整抓取**（含第6、15页补抓修复）。

## 楼层正文

### 1 楼（楼主）｜sobereva

谈谈隐式溶剂模型下溶解自由能和体系自由能的计算On the calculation of solvation free energy and system free energy via implicit solvent model



文/Sobereva @北京科音 First release: 2016-May-24   Last update: 2024-Jun-20



量子化学中，怎么在隐式溶剂模型下计算体系的溶解自由能和体系在溶剂下的自由能是频繁被问及的问题，其实计算方式非常简单，但很多人还是没有搞清楚或认识上有误区，这里就说说如何正确地计算。首先在第1节简要说一些和隐式溶剂模型有关的常识，这几个常识不知道的话是不可能正确进行计算的。关于溶剂模型更深入、系统的知识以及各种实际应用如pKa和氧化还原势的计算等，笔者会在相关中级量子化学教程（[已省略培训课程链接]）里非常全面具体地讲解，可另行参考。
...
谢谢sob老师指教。不过我没太理解老师的意思，我的体系弱相互作用主要就是氨基、羟基可能与水溶剂形成氢键，所以根据您的建议，(A) 可以用B3LYP不加D3 opt freq，然后双杂化泛函大基组级别下加D3(BJ)校正算高精度单点能，由此得比较精确的气相自由能，是这个意思吗？
...
2. 文中提到“溶质在溶剂环境下标况（298.15K、1M）时的自由能 = 溶质在1atm气相下的自由能 + 直接通过隐式溶剂模型计算的溶解自由能（始末都是1M） +1.89kcal/mol（标况下1atm->1M自由能变）”
...
水溶液中发生反应，在计算  “(2)关键词是B2PLYPD3/def2TZVP geom=allcheck，即使用较好的B2PLYP-D3(BJ)/def2-TZVP级别基于(1)的结构计算真空下较高精度电子能量”  这一步中计算真空下较高精度电子能量，关键词是否也应该加上SCRF=SMD？因为我看到58楼中sob老师回复：凡是液相的反应，建议优化和单点一律在溶剂模型下进行。

### 84 楼

leo1992 发表于 2020-5-14 16:55

水溶液中发生反应，在计算  “(2)关键词是B2PLYPD3/def2TZVP geom=allcheck，即使用较好的B2PLYP-D3(BJ)/ ...

计算真空下的能量显然不能带溶剂模型



58L说的是用简便方法计算溶剂下自由能，即不单独把溶解自由能的计算单拿出来的情况。

### 85 楼

本帖最后由 snljty 于 2020-5-23 21:19 编辑 
...
如果我用带SMD模型优化和计算频率，那么这里不是已经体现了一部分溶剂效应对自由能的贡献么？那再加上完整的M05-2X/6-31G*的溶解自由能，是不是有重复计算的部分呢？

还有您后面提到的，带SMD计算高精度单点能，可以作为真空高精度单点能+溶解自由能的近似，但是前者就没有热力学校正部分SMD对频率以及热力学数据的影响了，请问这也会引入误差么？谢谢。

不知道我表述清楚了没有

### 86 楼

snljty 发表于 2020-5-23 13:59

请问老师一下，有一点疑问。



如果我用带SMD模型优化和计算频率，那么这里不是已经体现了一部分溶剂效应 ...

SMD对结构和频率的影响（影响热力学校正量），以及对能量的影响需要分别看待



用SMD做opt freq，并没有体现SMD对能量的影响



用SMD算能量，也不体现SMD对结构和热力学校正量的影响

### 87 楼

sobereva 发表于 2020-5-24 08:22
...
本帖最后由 tianyongpan 于 2021-1-15 10:20 编辑 
...
本帖最后由 d1490541507 于 2021-6-28 15:49 编辑 
...
4)        How solvation Gibbs free energy were computed? If it was using the “standard” models by applying solvation terms to the ideal gas model (within harmonic oscillator-rigid rotor approximation) I am afraid that are not good enough to simulate solubility. First solvation shell and solvent entropic eﬀects need to be considered in order to have trustful results for solubility. The authors performed tests in order to clarify this issue?
...
guoxh 发表于 2021-7-15 13:39

sob老师：



您好，我是一个学习理论计算的硕士研究生，可以请教您一下关于分子溶解自由能计算的一些细节 ...

看你贴的链接的2(6)节就知道了，溶剂化熵不需要单独考虑。

但是审稿人说的第一配位层是否要加，这个需要结合你的体系来考虑，如果溶质和溶剂分子之间有比较强的氢键、静电相互作用或配位作用，那么需要用显式溶剂模型考虑第一配位层

### 119 楼

guoxh 发表于 2021-7-15 20:39
...
在文章里强调清楚你的溶解自由能的计算过程，并且引用Truhlar的SMD的原文。

并且说明，溶解自由能里面的熵效应在SMD模型拟合非极性部分参数的时候已经等效体现在参数里面了，所以直接用SMD模型按照作者的方式（即我博文里的方式）去计算溶解自由能，就已经能恰当地把熵效应等效表现出来了。

### 120 楼

sobereva 发表于 2021-7-16 15:52
...
wzkchem5 发表于 2021-7-15 22:32

看你贴的链接的2(6)节就知道了，溶剂化熵不需要单独考虑。

但是审稿人说的第一配位层是否要加，这个需要 ...

好哒，谢谢

### 122 楼

sob老师，我想请教一下，如果我想要研究的是单个离子（比如钠离子），在水溶剂中逐步与水分子形成团簇Na(H2O)n+过程中每一步的自由能变化，那么在第一步(Na+)+H2O=Na(H2O)+这个反应的自由能计算过程中，是否能对Na+采用您这篇帖子里介绍的方法结合scrf(smd,solvent=water)计算其溶解自由能呢，如果不行的话想请问老师怎么计算(Na+)+H2O=Na(H2O)+这个反应在水溶液中的自由能变化呢

### 123 楼

Qing 发表于 2021-8-5 10:02

sob老师，我想请教一下，如果我想要研究的是单个离子（比如钠离子），在水溶剂中逐步与水分子形成团簇Na(H2 ...

这实际上只是一个完全虚构的过程。对于当前目的，考察每一步的自由能变化，倒是可以

### 124 楼

sobereva 发表于 2021-8-6 08:06
...
请教各位：卢老师说“计算某分子在乙醇中的溶解自由能，就用比如# M052X/6-31G* scrf(SMD,solvent=ethanol)计算得到的单点能，减去# M052X/6-31G*计算得到的单点能”，那么用ORCA是不是可以这样计算：用DLPNO-CCSD(T) + SMD得到的单点能（优化时用SMD，计算DLPNO-CCSD(T)时也用SMD），减去不用SMD得到的DLPNO-CCSD(T)单点能（优化时不用SMD，计算DLPNO-CCSD(T)时也不用SMD），就得到溶解自由能？

### 126 楼

dnlx 发表于 2022-1-19 09:34

请教各位：卢老师说“计算某分子在乙醇中的溶解自由能，就用比如# M052X/6-31G* scrf(SMD,solvent=ethanol) ...

不建议这样

文中明确说了SMD在M05-2X/6-31G*下计算溶解自由能精度是最好的，结合DLPNO-CCSD(T)来用精度如何没法估计，被质疑都不好回答

### 127 楼

老师您好，我研究的是一个电子包合物，文献中“需要通过PCM 模型采用介电常数ε = 3.2 来模拟周围的冰环境”来看该条件下它的稳定情况，想问下老师这里应该如何设置这个冰环境？

### 128 楼

Lyy1 发表于 2022-2-23 20:18

老师您好，我研究的是一个电子包合物，文献中“需要通过PCM 模型采用介电常数ε = 3.2 来模拟周围的冰环境 ...
...
本帖最后由 作业抄 于 2022-3-1 17:06 编辑 



老师您好，我所研究的方向为钠离子电池电解液方向，比较常用到的就是Na+的脱溶剂化能。您在博文里所提到的物质的溶解自由能可以理解为我的Na盐溶解到电解液溶剂中并与周围的溶剂分子形成溶剂化结构所需要的能量，而脱溶剂化过程是Na+与周围的溶剂化分子分离。我想问的是，这里的溶解自由能与脱溶剂化过程所需要的能量是否是互为相反数，谢谢老师

### 130 楼

本帖最后由 作业抄 于 2022-3-1 17:12 编辑 



请问老师，利用隐形模型是否可以算出不同温度下的溶解自由能，博文中的两种说法是否是矛盾的

### 131 楼

作业抄 发表于 2022-3-1 10:10

请问老师，利用隐形模型是否可以算出不同温度下的溶解自由能，博文中的两种说法是否是矛盾的

不矛盾。第一张图是说得到精确的不同温度下的溶解自由能没戏，第二张图是说，在假设溶解自由能受温度的影响可忽略不计的情况下，可以近似地得到总自由能。两张图说的一个是溶解自由能，一个是总自由能，而且一个是不能精确计算、一个是可以近似计算。

### 132 楼

wzkchem5 发表于 2022-3-1 17:19

不矛盾。第一张图是说得到精确的不同温度下的溶解自由能没戏，第二张图是说，在假设溶解自由能受温度的影 ...

谢谢老师，我明白了

### 133 楼

作业抄 发表于 2022-3-1 17:02

老师您好，我所研究的方向为钠离子电池电解液方向，比较常用到的就是Na+的脱溶剂化能。您在博文里所提到的 ...

如果是脱溶剂化过程的自由能变，而且脱完之后处于标准浓度的理想气体状态，那么是相反数

### 134 楼

sobereva 发表于 2022-3-2 02:49

如果是脱溶剂化过程的自由能变，而且脱完之后处于标准浓度的理想气体状态，那么是相反数

谢谢老师，您这么一解释我就明白了。 老师您看，这是一篇Angew里面计算脱溶剂化的公式，我是不是可以先通过博文中1-4的输入文件计算Na+带有不同个溶剂分子的能量（在第一个输入文件添加溶剂分子），最后再用文献中的公式得出我想要的脱溶剂化能？

### 135 楼

作业抄 发表于 2022-3-2 09:33

谢谢老师，您这么一解释我就明白了。 老师您看，这是一篇Angew里面计算脱溶剂化的公式，我是不是可以先通 ...

可以。不过他那篇文章可能是用的电子能量求差算的，而非用的溶液下自由能算的（当然后者更严格），具体看它文章里怎么说

### 136 楼

sobereva 发表于 2022-3-2 22:07

可以。不过他那篇文章可能是用的电子能量求差算的，而非用的溶液下自由能算的（当然后者更严格），具体看 ...

谢谢老师，我明白了

### 137 楼

请问，如果我已经用隐式溶剂模型优化了分子并计算了振动频率，得到了溶液中分子的吉布斯自由能，为什么还要再算一遍溶解自由能呢？不会double count吗？

### 138 楼

Freeman 发表于 2022-3-5 11:21

请问，如果我已经用隐式溶剂模型优化了分子并计算了振动频率，得到了溶液中分子的吉布斯自由能，为什么还要 ...

只看单点能部分，你这样相当于高精度方法+SMD模型得到的溶剂中的单点能，本文说的算法是高精度方法+真空得到的单点能，加上溶解自由能，来计算溶剂下的单点能。两者目的上可以认为是在算同一个东西。Truhlar的意思是因为SMD模型在M05-2X/6-31G*级别下拟合的，所以原理上通常后者精度更高。
...
这得到的根本不是“溶液中分子的吉布斯自由能”

“隐式溶剂模型优化了分子并计算了振动频率”这个任务仅仅能得到自由能热校正量而已（虽然也给出电子能量，但由于opt freq用的级别通常太低，一般不会有人用这里给的电子能量）

### 140 楼

sob老师，我想问计算Na+带有多个溶剂分子时的能量，溶剂分子与Na+相对位置会对计算结果有影响吗，如果有影响该怎么确定溶剂分子与Na+的位置呢

### 141 楼

作业抄 发表于 2022-3-6 20:53

sob老师，我想问计算Na+带有多个溶剂分子时的能量，溶剂分子与Na+相对位置会对计算结果有影响吗，如果有影 ...

根据直觉搭建恰当的初始结构，然后优化得到



缺乏直觉的话，用genmer+molclus做团簇构型搜索得到，看
...
老师您好，我利用这篇博文里的四个输入文件计算了一个有机分子在不同基组下的能量，我想问一下，用第几个输出文件可以更好地表明我所计算的有机分子的LUHO和HOMO呢，我分别用这四个输出文件导入Multiwfn得到的LUMO和HOMO都不一样，谢谢老师

### 144 楼

作业抄 发表于 2022-4-26 16:09

老师您好，我利用这篇博文里的四个输入文件计算了一个有机分子在不同基组下的能量，我想问一下，用第几个输 ...

没有叫LUHO的东西



恰当级别优化，直接取同时产生的轨道能量就完了

轨道能级原理上没法观测，也没有绝对的精确与否。没法严格说用什么基组算的最合理
...
sob老师 我想请问一下 看文献中计算时 只说了在M062X/6-311++g(d,p)级别下计算单点能 是不是意味着他相当于省略了您例子中的2文件 直接用3文件算出的能量进行校正 
...
202205121534015279..png (206.65 KB, 下载次数 Times of downloads: 106)

下载附件 Download



2022-5-12 15:34 上传 Uploaded

### 147 楼

quantumlink 发表于 2022-5-12 15:33

sob老师 我想请问一下 看文献中计算时 只说了在M062X/6-311++g(d,p)级别下计算单点能 是不是意味着他相当于 ...

这就是我文中说的“一种粗略但比较省事的计算溶剂下焓或自由能的方法”

### 148 楼

sobereva 发表于 2022-5-14 11:36
...
请问一下算反应机理过程中，一个物种用了溶剂模型，那么所有的物种都得用吗？能在不同的物种上用不同的溶剂吗？

### 150 楼

mfdsrax2 发表于 2022-6-4 14:14

请问一下算反应机理过程中，一个物种用了溶剂模型，那么所有的物种都得用吗？能在不同的物种上用不同的溶剂 ...

不可以 溶剂模型相当于计算方法，必须一致

### 151 楼

mfdsrax2 发表于 2022-6-4 07:14

请问一下算反应机理过程中，一个物种用了溶剂模型，那么所有的物种都得用吗？能在不同的物种上用不同的溶剂 ...

一般必须统一，例外是如果你本来研究的就是两相过程，这时不用统一。比如计算辛醇/水分配系数，可以用辛醇里的自由能和水里的自由能相减。另一个例子是计算气体的溶解度，可以用溶剂里的自由能和气相的自由能相减。

### 152 楼

本帖最后由 Lym 于 2022-8-2 13:11 编辑 
...
可以计算激发态下的吉布斯自由能变嘛？，如果考虑了显性溶剂，此时这个显性溶剂是不是也应该是激发态下的呢？

### 153 楼

Lym 发表于 2022-8-2 09:31

老师，您好！



可以计算激发态下的吉布斯自由能变嘛？，如果考虑了显性溶剂，此时这个显性溶剂是不是也应 ...

可以



一般方式做激发态计算是对于整体而言的（除非用诸如背景电荷、片段有效势之类描述周围溶剂分子。到底算出来的激发态涉及什么分子的激发，有没有诸如溶质-溶剂间电荷转移等情况出现，需要用Multiwfn做空穴-电子分析等方式考察得到的激发态，没法自己指定只对哪些部分考虑激发
...
本帖最后由 Lym 于 2022-8-11 09:24 编辑 



老师，激发态下计算自由能的时候，是不是对应下面溶解自由能和气相下的的自由能的能量都应该在模板上加td？但是不知道为什么，在电脑上总是算的算的就不报错，然后就结束任务了

(2) td m062x/def2TZVP 

(3)td M052X/6-31G* 

(4)td M052X/6-31G* scrf(SMD,solvent=ethanol)

### 155 楼

Lym 发表于 2022-8-11 03:39

老师，激发态下计算自由能的时候，是不是对应下面溶解自由能和气相下的的自由能的能量都应该在模板上加td？ ...
...
另外，M06-2X算激发态通常是不好的选择，几乎总是高估激发能（除非算三重态激发能）

### 156 楼

sobereva 发表于 2022-8-11 06:47
...
老师，我主要是为了算反应前后的能量的自由能变。和基态进行对比，因为基态中基态气象自由能中使用的是m062x代替了双杂化泛函结合def2tzvp. 所以我这里激发态也采用了相同的泛函。
...
Lym 发表于 2022-8-11 09:34

老师，我主要是为了算反应前后的能量的自由能变。和基态进行对比，因为基态中基态气象自由能中使用的是m0 ...

既然关注点不在于激发能，那就也用和基态一样的

### 158 楼

本帖最后由 一条君 于 2022-9-12 13:31 编辑 



老师，“如果你不是用的SMD，而是其它溶剂模型结合dis cav rep选项来考虑溶剂非极性部分的贡献，那更没法严格地自定义新溶剂或者定义混合溶剂。”（1）这句话的意思是其他模型考虑非极性部分都是用一种参数，不区分溶剂的意思吗，比如统一用的谁的参数

（2）请问如果是不在内置的新溶剂，SMD不容易实现，用PCM模型是不是加上非极性部分比不加，算能量结果要好呢

（3）场景是算锂离子和极性聚合物PVDF、PEO的结合能，还是说这种情况只需要考虑极性部分就行呢，谢谢老师

### 159 楼

一条君 发表于 2022-9-12 13:29

老师，“如果你不是用的SMD，而是其它溶剂模型结合dis cav rep选项来考虑溶剂非极性部分的贡献，那更没法严 ...

即使用PCM溶剂模型时使用 “Dis Rep Cav” 考虑了非极性部分，而所使用的相应溶剂在高斯中并没有定义非极性部分的参数。其他均会报此错误。



解决：高斯或许可能有方法自定义 Dis Rep Cav 的参数，但未在说明书中给出。可以用SMD模型考虑非极性部分；或是不考虑非极性部分。

http://bbs.keinsci.com/thread-4829-1-1.html看来只能考虑极性部分了

### 160 楼

"碰上这种情况时的相对最佳的做法是写比如scrf(read,SMD,solvent=AAA)，然后照常定义eps和epsinf，这里AAA是与你当前要用的溶剂特征最接近的自带的溶剂"

老师，比如聚环氧乙烷作溶剂，那么AAA是否可以当作环氧乙烷呢，那聚偏二氟乙烯的AAA可以取谁呢，谢谢~

### 161 楼

一条君 发表于 2022-9-12 14:43

"碰上这种情况时的相对最佳的做法是写比如scrf(read,SMD,solvent=AAA)，然后照常定义eps和epsinf，这里AAA ...

看SMD计算非极性部分涉及的参数，取那些参数与当前溶剂最近的内置溶剂。或者那些参数如果对当前体系有现成的值，直接自定义设置非极性部分参数

### 162 楼

sobereva 发表于 2022-9-12 14:45

看SMD计算非极性部分涉及的参数，取那些参数与当前溶剂最近的内置溶剂。或者那些参数如果对当前体系有现 ...

老师，那这个前提可以用吗：比如聚环氧乙烷作溶剂，那么AAA是否可以当作环氧乙烷呢，

（看起来酸碱度应该接近，气液表面张力应该不一样吧，这样已经算最好的近似了吗）
...
ABQTrap 发表于 2022-12-7 11:16

各位老师好，我发现不同软件、不同溶剂模型内置的乙腈介电常数不一样。

如Gaussian中，乙腈介电常数ε=35. ...SMD模型自身定义了一批溶剂信息，见Minnesota Solvent Descriptor Database文档。不管什么程序，只要用的是SMD定义的溶剂，eps都是严格一样的。

35.688和36.6这点差异不会造成什么可查觉的影响。但建议用前者，Minnesota Solvent Descriptor Database文档里这个值都有确切的原始数据来源，可信度没有问题。

pubchem不算什么原始的参数来源。应当以专门的严谨的数据手册或者实验文章为准。

### 179 楼

sobereva 发表于 2022-12-8 10:38

SMD模型自身定义了一批溶剂信息，见Minnesota Solvent Descriptor Database文档。不管什么程序，只要用的是 ...

谢谢Sobereva老师！

### 180 楼

老师好，我计算一个甲醇溶液中的加氢反应，反应釜的温度160℃，压力是50barH2。请问计算过程中H2是否也需要按照教程中计算溶剂化自由呢？另外，热力学校正过程是否所有的反应物（包括氢气）都需要考虑上述反应的压力和温度呢？我做了相关的数据测试，反应势垒（TS-H2-Reactant）变化还是比较明显的，不知道取那个条件下的更合理呢?

### 181 楼

kevin 发表于 2023-3-27 18:40

老师好，我计算一个甲醇溶液中的加氢反应，反应釜的温度160℃，压力是50barH2。请问计算过程中H2是否也需要 ...

H2是在甲醇溶液环境中参与反应，所以H2也得计算它在溶剂环境中的自由能



温度肯定是要如实考虑的。在溶液中的反应要考虑反应物的实际浓度，浓度影响其自由能

### 182 楼

kevin 发表于 2023-3-27 11:40

老师好，我计算一个甲醇溶液中的加氢反应，反应釜的温度160℃，压力是50barH2。请问计算过程中H2是否也需要 ...

有两种做法：

（1）按sob老师说的，H2考虑溶解自由能。这样算出来的自由能，H2的标准态是1mol/L甲醇溶液

（2）H2不考虑溶解自由能，按气体算。这样算出来的自由能，H2的标准态是1bar气体。这样做的好处是，如果要把计算出来的自由能垒换算成反应速率，不需要查H2在这个温度下在甲醇里的溶解度。等价于计算“H2从气相溶解到甲醇里”和“溶解态的H2参与反应”两步的串联反应的总自由能垒。

两个自由能都有意义，但意义不同，只要注明H2的标准态，两个数即便差别很大也是都可以用的。

当然紧接着可以问一个问题：（1）和“H2的标准态是1mol/L甲醇溶液时精确的自由能垒”的差别，以及（2）和“H2的标准态是1bar气体时精确的自由能垒”的差别，哪个小？我感觉前者小（可能有一定误差抵消），但是我不确定

### 183 楼

sob老师，请问http://sobereva.com/327帖子里介绍的方法在pcm溶剂模型下计算G的话，有考虑熵效应对G的贡献吗？审稿人问我的Were those free energies only from the thermodynamic contributions or the contribution of TS was there also included?

### 184 楼

最瘦二师兄 发表于 2023-4-11 23:15

sob老师，请问http://sobereva.com/327帖子里介绍的方法在pcm溶剂模型下计算G的话，有考虑熵效应对G的贡献 ...

溶剂的熵效应在溶解自由能里体现了，溶质分子自身的熵效应在热力学校正量里体现了

### 185 楼

sobereva 发表于 2023-4-11 16:28

溶剂的熵效应在溶解自由能里体现了，溶质分子自身的熵效应在热力学校正量里体现了

如果是SMD模型的话确实如此，但是对于PCM，是不是仅当计算了非极性项（尤其是cavitation energy）的时候才包括了熵效应？因为PCM的极性项描述的是溶质和溶剂之间的静电作用能和极化作用能，没有熵的贡献。

### 186 楼

wzkchem5 发表于 2023-4-12 00:51

如果是SMD模型的话确实如此，但是对于PCM，是不是仅当计算了非极性项（尤其是cavitation energy）的时候 ...

是的。我默认了他用的是SMD。用PCM的话得再要求程序把非极性项考虑进去
...
请教一下w老师，在什么时候这个假设（溶解自由能受温度的影响可忽略不计）会成立，并由此得到的总自由能近似计算结果偏差不至于过大呢？

### 188 楼

Dolantin 发表于 2023-5-8 08:25

请教一下w老师，在什么时候这个假设（溶解自由能受温度的影响可忽略不计）会成立，并由此得到的总自由能 ...

你估计一下你的体系的溶解熵（例如根据文献类似体系报道的值来估计），以及你的研究涉及的温度变化范围，按G=H-TS估算溶解自由能随温度的变化，再根据你需要的精度判断这个变化幅度是否大于你需要的精度范围就行了。

### 189 楼

wzkchem5 发表于 2023-5-9 00:05

你估计一下你的体系的溶解熵（例如根据文献类似体系报道的值来估计），以及你的研究涉及的温度变化范围， ...

谢谢W老师！

### 190 楼

本帖最后由 ginlpein 于 2023-6-29 18:19 编辑 



sob老师，您文章中提到的都是自由能(G=E+自由能热校正量），但是在举例中差减的是电子能量(E)，我稍微有点疑惑。

请问：

对于同一个结构，计算所得G(隐式溶剂模型) - G(真空) = E(隐式溶剂模型) - E(真空) 一定是成立的吗？

最近对同一个结构，我分别在SMD模型和真空条件下做了频率分析，但是结果发现，自由能的差减结果会比电子能量之间差减小几个kJ/mol(不到1 kcal/mol)，两者自由能热校正量不相等，请问这个情况正常吗？

### 191 楼

ginlpein 发表于 2023-6-29 18:09

sob老师，您文章中提到的都是自由能(G=E+自由能热校正量），但是在举例中差减的是电子能量(E)，我稍微有点 ...

说清楚哪个例子



文中根本没用“G(隐式溶剂模型) - G(真空) = E(隐式溶剂模型) - E(真空) ”这种方式描述过，只说了用隐式溶剂模型算溶解自由能要用隐式溶剂模型下和真空下的电子能量求差，为什么非得这么算在文中解释得已经极其清楚了，没有任何歧义

### 192 楼

本帖最后由 一条君 于 2023-7-5 18:47 编辑 



老师，阅读了(8)关于单原子离子的溶解自由能的计算中提到的，Dixon等人的J. Phys. Chem. A, 105, 11534 (2001) 文献https://pubs.acs.org/doi/10.1021/jp012536s，请问这篇文章的delta Gsol(H3O+H2On-1)是在溶剂模型下计算的吉布斯自由能减去气相下计算的吉布斯自由能得到的吗（个人理解，不知是否正确），而不是在溶剂模型下计算的单点能减去气相下计算的单点能得到的了吧，谢谢老师。

### 193 楼

2532251334 发表于 2024-1-23 15:41

就是我直接自己先按体积比算好，然后用算好的参数就可以吗

有歧义，先按http://sobereva.com/620的要求仔细修改帖子，让你说的话没有歧义了，我们再回答你说的对不对

### 194 楼

2532251334 发表于 2024-1-23 22:41
...
I would like to mix water with some other solvents (toluene, DMF, etc.) in order to calculate my complex.

Through the link: "https://comp.chem.umn.edu/solvation/mnsddb.pdf" does eps mean epsilon and epsinf=refraction^2?

And if there are some parameters that are not available for water, those parameters of the other solvent (toluene, DMF, etc.) are used with 100% ratio?

thank you.

### 197 楼

moiumuio 发表于 2024-4-4 14:32

dear prof. 

I would like to mix water with some other solvents (toluene, DMF, etc.) in order to cal ...

eps=epsilon

When epsinf is not experimentally directly available, epsinf=refraction^2 is a good approximation

Water is special in SMD. You cannot use mixed solvent (in the case that non-polar parameters are also needed to be considered) containing water. If you need solvation free energy in this case, you can calculate solvation free energy for pure water and for another solvent respectively, and then take weighted average.

### 198 楼

sobereva 发表于 2024-4-4 17:01

eps=epsilon

When epsinf is not experimentally directly available, epsinf=refraction^2 is a good a ...

Thank you for your advice prof.

### 199 楼

请教下大家，我根据 溶质在溶剂环境下标况时的自由能 = 溶质在1atm气相下的自由能 + 直接通过隐式溶剂模型计算的溶解自由能 + 1.89kcal/mol 计算第一项溶质在1atm气相下的自由能 时，遭到审稿人质疑：“为什么气相下的自由能基于 隐式溶剂模型下opt freq得到的构型进行计算，而不是真空下的”。 我计算高精度单点时是没加隐式溶剂模型，是按照帖子中讲究的人的方法计算，除了sob提到的这些经验性的观点之外： “对于绝大多数中性分子，而且局部也不显离子性的情况，是否考虑隐式溶剂模型对几何结构以及振动频率的影响很小，用不用溶剂模型优化出来的结构的差异肉眼上甚至都看不出来。但是对于局部带很显著电荷的情况，不考虑溶剂模型优化出的结构可能与实际溶剂环境下其结构相差甚远、结果定性错误。如果你不清楚结构优化（以及随后的振动分析）时该不该加隐式溶剂模型，建议你一律加上，反正耗时也就增加一点（对于普通泛函做DFT而言），还可以避免对某些体系得到定性错误结果的风险，也避免一些的审稿人的吐槽。对几何结构的影响基本上都是溶剂的极性部分所致，是否考虑非极性部分对结构优化和振动分析总是很小的。还值得一提的是，有些反应，在溶剂下和气相下反应路径上的势能面差异极大。比如Menschutkin反应，在溶剂环境下才有过渡态，在气相下连过渡态都没有，这是因为这个反应的两个产物都显离子性，溶剂环境对二者稳定化程度十分显著。所以，研究溶液下化学反应过程，如果你心里没数，所有计算都应当加上隐式溶剂模型。//如前所述，对于某些情况气相和溶剂下结构差异甚大，甚至不用溶剂模型都优化不出某些结构的情况，还是应当在溶剂模型下优化结构再计算溶解自由能。如果你搞不清楚，就索性优化时候总是带着溶剂模型。//对于中性且没有局部显离子性的体系，带不带隐式溶剂模型对结构和自由能的热校正量影响甚微”。  是否有其他文献可以说明问题呢？我看帖子提到的文献是针对 溶解自由能(solvation free energy)在什么结构下计算。谢谢大家啦~

### 200 楼

Dolantin 发表于 2024-5-9 07:53

请教下大家，我根据 溶质在溶剂环境下标况时的自由能 = 溶质在1atm气相下的自由能 + 直接通过隐式溶剂模型 ...

把我这段话用作回复就完了

### 201 楼

sobereva 发表于 2024-5-10 07:34
...
各位老师好，大家有看着根据sob老师公式【溶质在溶剂下的自由能 = 溶质在1atm气相下的自由能 + 直接通过隐式溶剂模型计算的溶解自由能 + 1.89kcal/mol】计算自由能的文献吗？ 由于计算溶剂下的自由能遭到审稿人质疑，准备在正文和回复中引用相关文献进行说明，自己课题组的也可。一时没搜着严格按照此公式计算自由能的文献，回复审稿意见的时间稍紧，感激不尽

### 203 楼

Dolantin 发表于 2024-5-30 16:14

各位老师好，大家有看着根据sob老师公式【溶质在溶剂下的自由能 = 溶质在1atm气相下的自由能 + 直接通过隐 ...
...
对这点还心存疑虑的话，一定仔细去看J. Phys. Chem. A, 114, 13442 (2010)，包括其中补充材料部分，文章专门说了这个问题。

### 204 楼

sobereva 发表于 2024-5-31 09:52
...
各位老师好，我想问一下sob老师说的”一种粗略但比较省事的计算溶剂下焓或自由能的方法如下：(2)在(1)的结构下结合溶剂模型用高级别计算单点“中还用算在真空下的电子能量吗？为什么用这种方法就相当于是在高级别下计算了溶解自由能呢？用这种方法计算可以用于文章的发表吗？

### 206 楼

wang0108 发表于 2024-6-10 11:52

各位老师好，我想问一下sob老师说的”一种粗略但比较省事的计算溶剂下焓或自由能的方法如下：(2)在(1)的结 ...

sob老师已经说了，这种方法是计算“溶剂下焓或自由能”，也就是真空下自由能和溶解自由能之和，不是溶解自由能本身。如果要得到溶解自由能本身，那必须再做一个真空的计算

### 207 楼

wzkchem5 发表于 2024-6-10 19:19

sob老师已经说了，这种方法是计算“溶剂下焓或自由能”，也就是真空下自由能和溶解自由能之和，不是溶解 ...
...
sob老师，审稿人还是比较较真，我用您的观点在论文第一轮大修中回复了审稿人，还是没能说服他并让他理解，现在整到第二轮大修了。您有推荐的文献来支撑观点吗

### 209 楼

各位老师好，最近在进行氧化还原反应，想计算一下自由能，有几个问题想请教一下：

（1）反应物涉及到了氢氧根，我看到这篇文献J. Phys. Chem. B, 110, 16066 (2006)里面计算了质子、氢氧根等离子的溶解自由能，可以直接用吗？要是可以直接用的话气相中的自由能要怎么计算呢？

（2）最后生成物有水，溶剂是水，这样的水要怎么计算呢？

（3）对于有过渡金属的结构，比如Ni（OH）2，它可以直接用B3LYP/6-31G(d,p)来进行第一步的结构优化和振动分析吗，还是需要把过渡金属单独选用适合的基组

谢谢各位

### 210 楼

wang0108 发表于 2024-6-26 21:01

各位老师好，最近在进行氧化还原反应，想计算一下自由能，有几个问题想请教一下：

（1）反应物涉及到了氢 ...

1 可以。气相自由能平时该怎么算还怎么算。注意标准态转换问题就行了
...
2 按常规产物计算。注意如果你要计算考虑实际浓度的反应自由能的话，必须得考虑浓度校正，水的实际浓度和1M标准态浓度相距甚远。



3 这么小体系建议用PBE0/def2-TZVP。如果配体较大，基组改用更便宜的def-TZVP或6-311G**，而且此时若配体间或配体内色散作用对结构影响显著，还得再加D3或D4。

### 211 楼

sobereva 发表于 2024-6-27 03:38
...
请问老师，如果采用文中省事的方法算溶剂下焓或自由能，会被审稿人诟病吗 
...
screenshot-1721744550031.png (174.29 KB, 下载次数 Times of downloads: 62)

下载附件 Download



2024-7-23 22:22 上传 Uploaded

### 213 楼

kl19990215 发表于 2024-7-23 22:23
...
Dolantin 发表于 2024-6-11 09:58

sob老师，审稿人还是比较较真，我用您的观点在论文第一轮大修中回复了审稿人，还是没能说服他并让他理解 ...

如果结构不是很多，最直接的办法就是你把这两种情况都算一下，把结果扔给审稿人不就行了，审稿人可能是觉得你在偷懒不想算

### 215 楼

请问Gaussian16中scrf输入的关键字：用哪个关键词能让iefpcm同时包含d3？

### 216 楼

gaojiahao 发表于 2024-9-12 21:21
...
老师，用讲究的方法计算标况下气相下自由能，我先在隐式溶剂模型下优化（结构在气相和溶剂模型下差异较大），然后用优化后的结构做振动分析用于给Shermo程序作为输入文件，请问做振动分析时应不应该带隐式溶剂模型呢？

### 219 楼

喝了假咖啡 发表于 2024-10-18 20:50

老师，用讲究的方法计算标况下气相下自由能，我先在隐式溶剂模型下优化（结构在气相和溶剂模型下差异较大） ...

你既然计算气相下自由能，还带溶剂模型干嘛

### 220 楼

sobereva 发表于 2024-10-20 10:20

你既然计算气相下自由能，还带溶剂模型干嘛

老师，频率计算带溶剂模型无虚频，不带溶剂模型却出现2个虚频，在气相下重新优化消虚频那结构跟溶剂模型下优化的又不一样了。。。这种情况有什么好的解决方法吗？

### 221 楼

喝了假咖啡 发表于 2024-10-20 20:07

老师，频率计算带溶剂模型无虚频，不带溶剂模型却出现2个虚频，在气相下重新优化消虚频那结构跟溶剂模型 ...

实际是什么环境就在什么环境下计算，别管其它的，有虚频该怎么解决怎么解决
...
sob老师，我在结构优化的时候使用PCM溶剂模型，按照本文的方法定义了水和乙腈的混合溶剂模型，在计算能量的时候使用SMD溶剂模型，也定义了混合溶剂，请问对于两种溶剂模型的自定溶剂方法我需要引哪些参考文献
...
本帖最后由 一条君 于 2025-5-8 18:13 编辑 



老师，我要对比四氢呋喃和四氢吡喃 两种环醚的分子轨道，氧化电位，断键反应能等，smd溶剂模型里只有四氢呋喃的参数。

这种两个结构很相似的情况下，是用PCM去自定义介电常数更准确，还是两个都选取smd已有的四氢呋喃参数去计算呢，谢谢老师

（介电常数：四氢呋喃 7.6；四氢吡喃5.7）

### 225 楼

一条君 发表于 2025-5-8 18:08

老师，我要对比四氢呋喃和四氢吡喃 两种环醚的分子轨道，氧化电位，断键反应能等，smd溶剂模型里只有四氢呋 ...

如果只是横向对比，建议不考虑非极性部分，要不然会有偏向性，用IEFPCM结合各自的介电常数就够了。如果算反应能的绝对值，那就用四氢呋喃的非极性参数凑合。

### 226 楼

老师您好，审稿人问到了溶剂效应，但是我们并没有考虑，应该如何回复（做的是过渡金属加水分子前后的影响，但是实验是在真空条件下进行的）

### 227 楼

JJG 发表于 2025-5-18 22:05

老师您好，审稿人问到了溶剂效应，但是我们并没有考虑，应该如何回复（做的是过渡金属加水分子前后的影响， ...

既然实验是真空的就明确告诉审稿人，没必要考虑溶剂效应

### 228 楼

老师们好，我想问一下文献中计算了在气相和溶剂效应下某分子逐步与Li原子相互作用的ΔG，这里计算气相下ΔG需要特定的方法吗，还是就是结构优化然后计算高精度单点能就行，感谢老师！

### 229 楼

wang0108 发表于 2025-11-24 22:16

老师们好，我想问一下文献中计算了在气相和溶剂效应下某分子逐步与Li原子相互作用的ΔG，这里计算气相下ΔG ...

算涉及到的各个物质的自由能，然后求差，不会算自由能的话参考
...
PS：在相关初级量子化学教程（http://www.keinsci.com/KEQC）里有各种热力学量的背景知识和计算方法的特别系统的讲授

### 230 楼

sobereva 发表于 2025-11-25 07:54

算涉及到的各个物质的自由能，然后求差，不会算自由能的话参考

使用Shermo结合量子化学程序方便地计算分 ...

感谢Sob老师

### 231 楼

sob老师好，我刚看完楼上所有的帖子，但是对于氢离子、钠离子这些单原子离子的溶解自由基计算有些困惑。

以氢离子为例，气相中的氢离子的能量可以用推荐的G4直接获得

对于溶解自由能，按照图中的公式，使用一个显示分子，这里面的Gsol(HA+)，Gsol(A)和Ggas(H+)是直接用M05-2X(M06-2X)/6-31G* + SMD 计算得到的能量再相减，还是按照低级别获得零点校正能+高级别获得单点能分别得到图中的Gsol(HA+)，Gsol(A)和Ggas(H+)。
